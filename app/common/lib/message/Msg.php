...
class Msg{
    ...
    public $username; // 帐号
    public $userIp; // Ip
    public $validMinute = 60; // 短信有效时间(秒)
    ...
    public function checkSendNumber($key)
    {
        ...
        if ((time() - $Last_time) <= $this->validMinute) {
            $this->error = $this->validMinute . '秒内验证码不能重复发送';
            return false;
        }
    ...
    public function register()
    {
        if ($this->validateUsername() && $this->checkSendNumber(self::REGISTER_NAME . $this->username)) {
            ...
            //缓存验证码
            $redis->set(self::REGISTER_CODE.$this->username,123456);
    ...
    public function findPassword()
    {
        if ($this->validateUsername() && $this->checkSendNumber(self::FIND_NAME . $this->username)) {
            ...
            //缓存验证码
            $redis->set(self::FIND_CODE.$this->username,654321);
